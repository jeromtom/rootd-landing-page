...
async function addToGoogleSheets(data, env) {
    try {
        console.log('Starting Google Sheets integration for:', data.clinicName);
        ...
        const sheetsResponse = await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${env.GOOGLE_SHEETS_SHEET_ID}/values/A:J:append?valueInputOption=RAW`, {
            method: 'POST',
            headers: {
        ...
        if (!sheetsResponse.ok) {
            ...
            throw new Error(`Failed to add data to Google Sheets: ${sheetsResponse.status} - ${errorText}`);
        }
        const sheetsData = await sheetsResponse.json();
        console.log('Successfully added to Google Sheets:', data.clinicName, sheetsData);
        return { success: true };
    }
    catch (error) {
...
async function createJWT(env) {
    try {
        ...
        let privateKeyPem = env.GOOGLE_SHEETS_PRIVATE_KEY;
        ...
        const privateKey = await crypto.subtle.importKey('pkcs8', new TextEncoder().encode(privateKeyPem), {
...
async function handleWaitlistRequest(request, env, ctx) {
...
export default {
    async fetch(request, env, ctx) {
        ...
        if (url.pathname === '/api/waitlist') {
            return handleWaitlistRequest(request, env, ctx);
        }
        // Handle root path
        if (url.pathname === '/') {
            return new Response('Rootd Waitlist API', { status: 200 });
        }
